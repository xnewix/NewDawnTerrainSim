...
public:
	...
	double findmax(std::vector<double>& tmp) {
		double max = 0.0;
	...
	double findpercentage(std::vector<double>& tmp) {
		double value = findmax(tmp) / 255.0;
		return value;
	}
	
	void findcolor(std::vector<double>& tmp) {
		
		double percentage = findpercentage(tmp);

		for (int i = 0; i<tmp.size(); i++) {
			ColorValues.push_back(tmp[i] / percentage);
		}
	}
	
	void outputdata(std::vector<double>& tmp) {
		std::ofstream datafile;
		datafile.open("heightvaluesdata.txt");
		datafile << "Heightdata: " << tmp.size() << std::endl;
		
		for(int i=0; i<tmp.size(); i++) {
			datafile << i << ":  " << tmp[i] << std::endl;		
		}
